...
export async function handleAIResponse(sender: string, message: string, botId: string) {
  ...
  const messages: any[] = (history || []).reverse().map(h => ({
  ...
  try {
    const { text } = await generateText({
      ...
      maxSteps: 3, // Allow some tool iteration
    });
    ...
    // 6. Log Memory (Both user and AI response)
    ...
    // 7. Background: Save Sales Insights
    // (Non-blocking insight generation can be added here)

    return text;
  } catch (claudeErr: any) {
    console.error(`🔥 [AI Engine] Claude API Error: ${claudeErr.message}`);
    
    // Fallback: Even if Claude fails, log the user message so we don't lose context
